...
- **본문 원문**: 낭독/단어예습 탭에 쓰이는 실제 지문 전체가 아직
  전달되지 않았습니다. `src/data/sentences_L01.json`에는
  `grammar_L01.json`에서 `source:"text"`로 표시된, 본문에서 실제로
  발췌된 문장만 반영되어 있고 나머지는 비어 있습니다(각 섹션 상단에
  "원문 확정 전" 배너로 표시됨). 원문이 도착하면 이 JSON 파일(또는
  Supabase 연동 후 관리자 화면의 콘텐츠 인라인 편집)에 문장을 채우고,
  각 섹션의 `"complete": false`를 `true`로 바꿔주세요.
- **ElevenLabs 음원**: 위 "Supabase 연동 방법" 5번 참고.
